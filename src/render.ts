    ...
    <html lang="en">
      ...
      <body>
        <main class="app-shell">
          <header class="topbar">
            <a class="brand" href="/">Hono D1 Starter</a>
            ${props.user
...
export function loginPage(result?: HtmlEscapedString | string): HtmlEscapedString {
  return html`<section class="stack" x-data="{ email: '' }">
    <header>
      <h1>Sign in</h1>
    ...
    <div id="login-response">${result ?? ""}</div>
...
export function loginResult(
  ...
  if (props.kind === "dev") {
    return html`<div class="notice" role="status">
      <p><strong>Dev login link ready.</strong></p>
      <p>The link for ${props.email} is available in this UI.</p>
      <p><a href="${props.link}">Open magic link</a></p>
  ...
  return html`<div class="notice" role="alert">
    <p><strong>Could not continue.</strong></p>
    <p>${props.message}</p>
  </div>` as HtmlEscapedString;
}
